...
public class CommandsAdmin extends Commands {
    ...
    public static CommandResponse configResetRanksCommand(ICommandSender sender, List<String> args) {
        MyTown.instance.getRanksConfig()
            .create(new Rank.Container());
        ChatManager.send(
            sender,
            "mytown.cmd.config.reset",
            MyTown.instance.getRanksConfig()
                .getName());
        return CommandResponse.DONE;
    }
    ...
    public static CommandResponse configResetFlagsCommand(ICommandSender sender, List<String> args) {
        MyTown.instance.getFlagsConfig()
            .create(new ArrayList<FlagsConfig.Wrapper>());
        ChatManager.send(
            sender,
            "mytown.cmd.config.reset",
            MyTown.instance.getFlagsConfig()
    ...
    public static CommandResponse blocksInfoCommand(ICommandSender sender, List<String> args) {
        ...
        IChatComponent header = LocalManager
            .get("myessentials.format.list.header", new ChatComponentFormatted("{9|BLOCKS}"));
        ...
        extraBlocksSources.add(
            LocalManager.get(
                "mytown.notification.blocks.info.extra",
                new ChatComponentFormatted("{9|TOWN}"),
                town.townBlocksContainer.getExtraBlocks()));
        for (Resident res : town.residentsMap.keySet()) {
            extraBlocksSources
                .add(LocalManager.get("mytown.notification.blocks.info.extra", res, res.getExtraBlocks()));
        }

        ChatManager.send(
            sender,
            "mytown.notification.blocks.info",
            header,
            blocks,
            extraBlocks,
            extraBlocksSources,
    ...
    public static CommandResponse townBlocksMaxSetCommand(ICommandSender sender, List<String> args) {
        ...
        ChatManager
            .send(sender, "mytown.notification.town.blocks.extra.set", town.townBlocksContainer.getExtraBlocks(), town);
        return CommandResponse.DONE;
    }
    ...
    public static CommandResponse townBlocksFarclaimsSetCommand(ICommandSender sender, List<String> args) {
        ...
        ChatManager.send(
            sender,
            "mytown.notification.town.blocks.farClaims.set",
            town.townBlocksContainer.getExtraFarClaims(),
            town);
        return CommandResponse.DONE;
    }
    ...
    public static CommandResponse permTownSetCommand(ICommandSender sender, List<String> args) {
        ...
        if (!flag.flagType.configurable && (playerSender == null || !PermissionProxy.getPermissionManager()
            .hasPermission(
                playerSender.getPersistentID(),
                "mytown.adm.cmd.perm.town.set." + flag.flagType + ".bypass"))) {
    ...
    public static CommandResponse permTownToggleCommand(ICommandSender sender, List<String> args) {
        ...
        if (!flag.flagType.configurable && (playerSender == null || !PermissionProxy.getPermissionManager()
            .hasPermission(
                playerSender.getPersistentID(),
                "mytown.adm.cmd.perm.town.toggle." + flag.flagType + ".bypass"))) {
            throw new MyTownCommandException("mytown.cmd.err.flag.unconfigurable", args.get(1));
        } else {
            if (flag.toggle()) {
                ChatManager.send(sender, "mytown.notification.perm.success");
    ...
    public static CommandResponse permWildSetCommand(ICommandSender sender, List<String> args) {
        ...
        MyTown.instance.getWildConfig()
            .write(Wild.instance.flagsContainer);
        return CommandResponse.DONE;
    }
    ...
    public static CommandResponse claimCommand(ICommandSender sender, List<String> args) {
        ...
        if (args.size() < 2) {
            ...
            if (!CommandsAssistant.checkNearby(player.dimension, player.chunkCoordX, player.chunkCoordZ, town)) { // Checks
                                                                                                                  // if
                                                                                                                  // the
                                                                                                                  // player
                                                                                                                  // can
            ...
            TownBlock block = getUniverse()
                .newBlock(player.dimension, player.chunkCoordX, player.chunkCoordZ, isFarClaim, 0, town);
            if (block == null) {
            ...
            List<ChunkPos> chunks = WorldUtils.getChunksInBox(
                player.dimension,
            ...
            for (Iterator<ChunkPos> it = chunks.iterator(); it.hasNext();) {
                ...
                if (getUniverse().blocks.contains(player.dimension, chunk.getX(), chunk.getZ())) it.remove();
            }
            if (isFarClaim) {
                ChatManager.send(sender, "mytown.adm.cmd.far.claim");
            }
            ...
            for (ChunkPos chunk : chunks) {
                TownBlock block = getUniverse()
                    .newBlock(player.dimension, chunk.getX(), chunk.getZ(), isFarClaim, 0, town);
        ...
        if (sender instanceof EntityPlayer) {
            ...
            if (player.inventory.getCurrentItem() != null) {
                ...
                if (player.inventory.getCurrentItem()
                    ...
                    Block block = ((ItemBlock) player.inventory.getCurrentItem()
                        .getItem()).field_150939_a;
                    list.add(block.getClass());
                    if (block instanceof ITileEntityProvider) {
                        TileEntity te = ((ITileEntityProvider) block).createNewTileEntity(
                ...
                ChatManager.send(
                    ...
                    new ChatComponentText(
                        "For item: " + player.inventory.getCurrentItem()
                            .getDisplayName()));
                for (Class cls : list) {
    ...
    public static class Plots {
        ...
        public static CommandResponse plotPermSetCommand(ICommandSender sender, List<String> args) {
            if (args.size() < 4) return CommandResponse.SEND_SYNTAX;
        ...
        public static CommandResponse plotPermToggleCommand(ICommandSender sender, List<String> args) {
            if (args.size() < 3) return CommandResponse.SEND_SYNTAX;
        ...
        public static CommandResponse plotInfoCommand(ICommandSender sender, List<String> args) {
            ...
            IChatComponent header = LocalManager
                .get("myessentials.format.list.header", new ChatComponentFormatted("{9|%s}", plot.getName()));
            ChatManager.send(
                sender,
                "mytown.format.plot.long",
                plot.ownersContainer,
                plot.toVolume()
                    .toChatMessage());
            return CommandResponse.DONE;
        }
