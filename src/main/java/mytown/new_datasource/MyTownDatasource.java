...
public class MyTownDatasource extends DatasourceSQL
{
  ...
  public boolean saveTown( Town town )
  {
    ...
    try
    {
      if( getUniverse().towns.contains( town ) )
      { // Update
  ...
  public boolean saveBlock( TownBlock block )
  {
    ...
    try
    {
      if( getUniverse().blocks.contains( block ) )
      { // Update
  ...
  { // TODO Insert any new permissions to the RankPermission table
    LOG.debug( "Saving Rank {} in town {}", rank.getName(), rank.getTown().getName() );
    try
    {
      if( getUniverse().ranks.contains( rank ) )
      { // Update
        try
        {
  ...
  public boolean saveResident( Resident resident )
  {
    ...
    try
    {
      if( getUniverse().residents.contains( resident.getUUID() ) )
      { // Update
  ...
  public boolean savePlot( Plot plot )
  {
    ...
    try
    {
      if( getUniverse().plots.contains( plot ) )
      { // Update
