...
public class CommandsEveryone extends Commands {
    ...
    @Command(name = "mytown", permission = "mytown.cmd", alias = { "t", "town" }, syntax = "/town <command>")
    ...
    public static CommandResponse leaveCommand(ICommandSender sender, List<String> args) {
        ...
        if (town.residentsMap.get(res) != null && town.residentsMap.get(res)
            .getType() == Rank.Type.MAYOR) {
    ...
    public static CommandResponse spawnCommand(ICommandSender sender, List<String> args) {
        ...
        if (res.getTeleportCooldown() > 0) {
            throw new MyTownCommandException(
                "mytown.cmd.err.spawn.cooldown",
                res.getTeleportCooldown(),
    ...
    public static CommandResponse blocksListCommand(ICommandSender sender, List<String> args) {
        ...
        root.appendSibling(
            LocalManager.get("myessentials.format.list.header", new ChatComponentFormatted("{9|CLAIMS}")));
    ...
    public static CommandResponse blocksInfoCommand(ICommandSender sender, List<String> args) {
        ...
        for (Resident resident : town.residentsMap.keySet()) {
            extraBlocksSources
                .add(LocalManager.get("mytown.notification.blocks.info.extra", resident, resident.getExtraBlocks()));
        }
    ...
    public static class Plots {
        ...
        public static CommandResponse plotPermSetCommand(ICommandSender sender, List<String> args) {
            ...
            if (!plot.ownersContainer.contains(res) && !plot.getTown()
                .hasPermission(res, "mytown.bypass.plot")) {
        ...
        public static CommandResponse plotDeleteCommand(ICommandSender sender, List<String> args) {
            ...
            if (sender instanceof EntityPlayer) {
                ...
                world = MinecraftServer.getServer()
                    .worldServerForDimension(plot.getDim());
            }
    ...
    public static CommandResponse listRanksCommand(ICommandSender sender, List<String> args) {
        ...
        IChatComponent root = new ChatComponentList();
        root.appendSibling(
            LocalManager.get("myessentials.format.list.header", new ChatComponentFormatted("{9|RANKS}")));
        for (Rank rank : town.ranksContainer) {
            root.appendSibling(
                new ChatComponentFormatted("{7| - }")
                    .appendSibling(LocalManager.get("mytown.format.rank.long", rank.getName(), rank.getType())));
        }
    ...
    public static CommandResponse bankAmountCommand(ICommandSender sender, List<String> args) {
        ...
        ChatManager.send(
            sender,
            "mytown.notification.town.bank.info",
            EconomyProxy.getCurrency(town.bank.getAmount()),
            EconomyProxy.getCurrency(town.bank.getNextPaymentAmount()));
        return CommandResponse.DONE;
    }
