...
public class Bank {
    ...
    public void payUpkeep() {
        ...
        if (makePayment(amount)) {
            ...
            daysNotPaid++;
            town.notifyEveryone(
                MyTown.instance.LOCAL.getLocalization(
                    "mytown.notification.town.upkeep.failed",
                    Config.instance.upkeepTownDeletionDays.get() - daysNotPaid));
        }
    }
    ...
    public int getNextPaymentAmount() {
        return (Config.instance.costTownUpkeep.get()
            + Config.instance.costAdditionalUpkeep.get() * town.townBlocksContainer.size()) * (1 + daysNotPaid);
    }
