...
public class Plot implements IChatFormat {
    ...
    public boolean hasPermission(Resident res, FlagType<Boolean> flagType) {
        ...
        boolean permissionBypass = PermissionProxy.getPermissionManager()
    ...
    public void deleteSignBlocks(SignType signType, World world) {
        if (world.provider.dimensionId != dim) return;
    ...
    public static class Container extends ArrayList<Plot> implements IChatFormat {
        ...
        public Plot get(Resident res) {
            return get(
                ...
                (int) Math.floor(res.getPlayer().posX),
                (int) Math.floor(res.getPlayer().posY),
                (int) Math.floor(res.getPlayer().posZ));
        }
        ...
        public List<Plot> getPlotsOwned(Resident res) {
            ...
            for (Plot plot : this) {
                if (plot.ownersContainer.contains(res)) list.add(plot);
            }
            return list;
        }
        ...
        public int getAmountPlotsOwned(Resident res) {
            int plotsOwned = 0;
            for (Plot plot : this) {
                if (plot.ownersContainer.contains(res)) plotsOwned++;
            }
            return plotsOwned;
        }
