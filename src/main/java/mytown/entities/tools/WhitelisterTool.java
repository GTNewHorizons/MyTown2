...
public class WhitelisterTool extends Tool {
    ...
    public WhitelisterTool(Resident owner) {
        super(
            owner.getPlayer(),
            LocalManager.get("myessentials.tool.name", LocalManager.get("mytown.tool.whitelister.name"))
    ...
    protected String[] getDescription() {
        return LocalManager
            .get(
                "mytown.tool.whitelister.description",
                flagType == null ? LocalManager.get("mytown.tool.whitelister.removal")
                    .getUnformattedText() : flagType.toString())
            .getLegacyFormattedText();
    }

    @Override
    public void onShiftRightClick() {
        if (flagType == FlagType.getWhitelistable()
            .get(
                FlagType.getWhitelistable()
                    .size() - 1)) {
            flagType = null;
            updateDescription();
            ChatManager.send(
                ...
                LocalManager.get("mytown.tool.whitelister.removal")
                    .getUnformattedText());
        } else {
            if (flagType == null) {
                flagType = FlagType.getWhitelistable()
                    .get(0);
            } else {
                flagType = FlagType.getWhitelistable()
                    .get(
    ...
    protected boolean hasPermission(Town town, BlockPos bp) {
        ...
        if (!(town.residentsMap.get(owner)
            .getName()
            .equals("Assistant")
            || town.residentsMap.get(owner)
                .getName()
                .equals("Mayor"))) {
