...
public class GetterDynamic extends Getter {
    ...
    public Object invoke(Class<?> returnType, Object instance, Object... parameters) throws GetterException {
        ...
        for (Caller caller : callers) {
            try {
                ...
                throw new GetterException(
                    "Failed to access field/method " + caller.getName() + " in getter: " + name,
                    iaex);
            } catch (NoSuchMethodException nmex) {
                throw new GetterException("Failed to get method " + caller.getName() + " in getter: " + name, nmex);
            } catch (InvocationTargetException itex) {
                throw new GetterException(
                    "Failed to get " + returnType.getSimpleName() + " type in getter: " + name,
                    itex);
            } catch (NumberFormatException numex) {
                throw new GetterException(
                    "Failed to get Integer type in getter: " + name + ". Found " + caller.getName(),
                    numex);
            } catch (IndexOutOfBoundsException iobex) {
                throw new GetterException(
                    "Integer out of bounds in getter: " + name + ". Found " + caller.getName(),
                    iobex);
            } catch (Exception ex) {
            ...
            if (lastInstance == null) {
                throw new GetterException(
                    "Failed to get " + returnType.getSimpleName()
                        + " type in getter: "
                        + name
                        + ". Value returned null.");
            }
        ...
        if (lastCaller.getValueType() != null && !lastCaller.getValueType()
            .isAssignableFrom(lastInstance.getClass())) {
            throw new GetterException(
                "Failed to get " + lastCaller.getValueType()
                    .getSimpleName() + " type in getter: " + name);
        }
