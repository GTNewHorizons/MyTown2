...
public class SellSign extends Sign {
    ...
    public SellSign(TileEntitySign te, NBTTagCompound signData) {
        ...
        this.plot = MyTownUniverse.instance.plots
            .get(te.getWorldObj().provider.dimensionId, te.xCoord, te.yCoord, te.zCoord);
    }
    ...
    public void onRightClick(EntityPlayer player) {
        ...
        if (EconomyProxy.getEconomy()
            .takeMoneyFromPlayer(resident.getPlayer(), price)) {
            for (Resident resInPlot : plot.ownersContainer) {
                ChatManager.send(
                    resInPlot.getPlayer(),
                    "mytown.notification.plot.buy.oldOwner",
                    plot,
            ...
            if (!plot.getTown().residentsMap.containsKey(resident)) {
                MyTown.instance.datasource
                    .linkResidentToTown(resident, plot.getTown(), plot.getTown().ranksContainer.getDefaultRank());
            }
    ...
    protected String[] getText() {
        ...
        return new String[] { MyTown.instance.LOCAL.getLocalization("mytown.sign.sell.title")
            .getUnformattedText(),
            MyTown.instance.LOCAL.getLocalization("mytown.sign.sell.description.owner")
                .getUnformattedText() + " " + owner.getPlayerName(),
            MyTown.instance.LOCAL.getLocalization("mytown.sign.sell.description.price")
                .getUnformattedText() + price,
    ...
    public static class SellSignType extends SignType {
        ...
        public boolean isTileValid(TileEntitySign te) {
            ...
            try {
                ...
                if (!rootTag.getString("Type")
                    .equals(SellSignType.instance.getTypeID())) return false;

                NBTBase data = rootTag.getTag("Value");
                if (!(data instanceof NBTTagCompound)) return false;
