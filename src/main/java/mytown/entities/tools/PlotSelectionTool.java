...
public class PlotSelectionTool extends Tool {
    ...
    public PlotSelectionTool(Resident owner, String plotName) {
        super(
            owner.getPlayer(),
            LocalManager.get("myessentials.tool.name", LocalManager.get("mytown.tool.plot.selection.name"))
    ...
    public void onItemUse(BlockPos bp, int face) {
        ...
        if (selectionFirst == null) {
            ...
            if (owner.getPlayer() instanceof EntityPlayerMP) {
                VisualsHandler.instance.markBlock(
                    bp.getX(),
                    bp.getY(),
                    bp.getZ(),
                    bp.getDim(),
    ...
    protected String[] getDescription() {
        return LocalManager.get("mytown.tool.plot.selection.description", plotName, heightDependent)
    ...
    public void onShiftRightClick() {
        ...
        ChatManager.send(
            owner.getPlayer(),
            "myessentials.tool.mode",
            LocalManager.get("mytown.tool.plot.selection.property"),
            heightDependent);
    }
            ...
            if (delay <= 0) {
                ...
                try {
                    new DelayedThread(
                        ...
                        VisualsHandler.instance,
                        owner.getPlayer(),
                        owner.getPlayer()).start();
                } catch (Exception ex) {
    ...
    protected boolean hasPermission(Town town) {
        if (town == null || town != owner.townsContainer.getMainTown() && selectionFirst != null
            || selectionFirst != null && town != selectionFirst.town) {
    ...
    private boolean expandVertically() {
        if (selectionFirst == null || selectionSecond == null) return false;

        selectionFirst.y = 0;
        selectionSecond.y = MinecraftServer.getServer()
            .worldServerForDimension(selectionSecond.dim)
            .getActualHeight() - 1;

        if (owner.getPlayer() instanceof EntityPlayerMP)
            VisualsHandler.instance.unmarkBlocks((EntityPlayerMP) owner.getPlayer(), owner.getPlayer());

        if (owner.getPlayer() instanceof EntityPlayerMP) VisualsHandler.instance.markPlotBorders(
            selectionFirst.x,
            selectionFirst.y,
            selectionFirst.z,
            selectionSecond.x,
    ...
    private void createPlotFromSelection() {
        ...
        Plot plot = MyTownUniverse.instance.newPlot(
            plotName,
            selectionFirst.town,
            selectionFirst.dim,
            selectionFirst.x,
    ...
    private void normalizeSelection() {
        if (!heightDependent) {
            ...
            if (owner.getPlayer() instanceof EntityPlayerMP) VisualsHandler.instance.markCorners(
                selectionFirst.x,
                selectionFirst.y,
                selectionFirst.z,
                selectionSecond.x,
        ...
        if (!(selectionFirst.town instanceof AdminTown)) {
            if ((Math.abs(selectionFirst.x - selectionSecond.x) + 1)
                * (Math.abs(selectionFirst.z - selectionSecond.z) + 1) < Config.instance.minPlotsArea.get()
                ...
                ChatManager.send(
                    owner.getPlayer(),
                    "mytown.cmd.err.plot.tooSmall",
                    Config.instance.minPlotsArea.get(),
                ...
                * (Math.abs(selectionFirst.z - selectionSecond.z) + 1) > Config.instance.maxPlotsArea.get()
                || Math.abs(selectionFirst.y - selectionSecond.y) + 1 > Config.instance.maxPlotsHeight.get()) {
                    ChatManager.send(
                        owner.getPlayer(),
                        "mytown.cmd.err.plot.tooLarge",
                        Config.instance.maxPlotsArea.get(),
    ...
    private class Selection {
        ...
        public Selection(int dim, int x, int y, int z) {
            ...
            this.town = MyTownUniverse.instance.blocks.get(dim, x >> 4, z >> 4)
                .getTown();
        }
    }
}
