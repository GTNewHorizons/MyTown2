...
import mytown.api.events.NationEvent;
import mytown.api.events.PlotEvent;
import mytown.api.events.RankEvent;
import mytown.api.events.ResidentEvent;
import mytown.api.events.TownBlockEvent;
...
public class MyTownUniverse { // TODO Allow migrating between different Datasources
    ...
    private void configureTown(Town town, Resident creator) {
        ...
        town.setSpawn(
            new Teleport(
                creator.getPlayer().dimension,
                (float) creator.getPlayer().posX,
                (float) creator.getPlayer().posY,
                (float) creator.getPlayer().posZ,
                creator.getPlayer().cameraYaw,
                creator.getPlayer().cameraPitch));

        // Saving town to database
        if (!getDatasource().saveTown(town)) throw new CommandException("Failed to save Town");
        ...
        if (!(town instanceof AdminTown)) {
            ...
            if (!getDatasource().linkResidentToTown(creator, town, town.ranksContainer.getMayorRank())) {
                MyTown.instance.LOG
                    .error("Problem linking resident {} to town {}", creator.getPlayerName(), town.getName());
            }
    ...
    public final TownBlock newBlock(int dim, int x, int z, boolean isFarClaim, int pricePaid, Town town) {
        ...
        if (TownBlockEvent.fire(new TownBlockEvent.BlockCreateEvent(block))) return null;
        return block;
    }
    ...
    public final Rank newRank(String name, Town town, Rank.Type type) {
        ...
        if (RankEvent.fire(new RankEvent.RankCreateEvent(rank))) return null;
        return rank;
    }
    ...
    public final Resident newResident(UUID uuid, String playerName, boolean isFakePlayer) {
        ...
        if (ResidentEvent.fire(new ResidentEvent.ResidentCreateEvent(resident))) return null;
        return resident;
    }
    ...
    public final Plot newPlot(String name, Town town, int dim, int x1, int y1, int z1, int x2, int y2, int z2) {
        ...
        if (PlotEvent.fire(new PlotEvent.PlotCreateEvent(plot))) return null;
        return plot;
    }
    ...
    public final Nation newNation(String name) {
        ...
        if (NationEvent.fire(new NationEvent.NationCreateEvent(nation))) return null;
        return nation;
    }
    ...
    public final void removePlot(Plot plot) {
        ...
        for (Plot p : plots) {
            if (p.getName()
                .equals(plot.getName())) removeFromCompletionMap = false;
        }
        if (removeFromCompletionMap) CommandCompletion.removeCompletion("plotCompletion", plot.getName());
