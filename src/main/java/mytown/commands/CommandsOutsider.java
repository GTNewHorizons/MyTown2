...
public class CommandsOutsider extends Commands {
    ...
    public static CommandResponse infoCommand(ICommandSender sender, List<String> args) {
        ...
        for (Town town : towns) {
            IChatComponent header = LocalManager
                .get("myessentials.format.list.header", new ChatComponentFormatted("{9|%s}", town.getName()));
            ChatManager.send(
                sender,
                "mytown.format.town.long",
                header,
                town.residentsMap.size(),
                town.townBlocksContainer.size(),
                town.getMaxBlocks(),
                town.plotsContainer.size(),
                town.residentsMap,
                town.ranksContainer);
        }
    ...
    public static CommandResponse resCommand(ICommandSender sender, List<String> args) {
        ...
        ChatManager.send(
            sender,
            "mytown.format.resident.long",
            header,
            res.townsContainer,
            Formatter.formatDate(res.getJoinDate()),
            Formatter.formatDate(res.getLastOnline()),
            res.getExtraBlocks());
        return CommandResponse.DONE;
    }
    ...
    public static CommandResponse newTownCommand(ICommandSender sender, List<String> args) {
        ...
        for (int x = ((int) player.posX >> 4) - Config.instance.distanceBetweenTowns.get(); x
            <= ((int) player.posX >> 4) + Config.instance.distanceBetweenTowns.get(); x++) {
            for (int z = ((int) player.posZ >> 4) - Config.instance.distanceBetweenTowns.get(); z
                <= ((int) player.posZ >> 4) + Config.instance.distanceBetweenTowns.get(); z++) {
    ...
    public static CommandResponse pricesCommand(ICommandSender sender, List<String> args) {
        ...
        IChatComponent header = LocalManager
            .get("myessentials.format.list.header", new ChatComponentFormatted("{9|PRICES}"));
        ChatManager.send(
            sender,
            "mytown.notification.prices",
            header,
