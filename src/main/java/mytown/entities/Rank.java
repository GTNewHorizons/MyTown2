...
public class Rank implements IChatFormat {
    ...
    public IChatComponent toChatMessage() {
        return LocalManager.get("mytown.format.rank", name)
            .setChatStyle(new ChatStyle().setColor(type.color));
    }
    ...
    public static class Serializer extends SerializerTemplate<Rank> {
        ...
        public Rank deserialize(JsonElement json, java.lang.reflect.Type typeOfT, JsonDeserializationContext context)
            ...
            String name = jsonObject.get("name")
                .getAsString();
            Rank.Type rankType = Type.valueOf(
                jsonObject.get("type")
                    .getAsString());
            Rank rank = new Rank(name, null, rankType);
            if (jsonObject.has("permissions")) {
                rank.permissionsContainer.addAll(
