...
public abstract class Segment {
    ...
    public Resident getOwner(Object object) {
        ...
        try {
            ...
            ownerObj = getters.get("owner")
                .invoke(Object.class, object, object);
    ...
    public static class Serializer extends SerializerTemplate<Segment> {
        ...
        private void serializeItem(SegmentItem segment, JsonObject json, JsonSerializationContext context) {
            ...
            if (segment.inventoryUpdate != null) {
                JsonObject jsonUpdate = new JsonObject();
                if (segment.inventoryUpdate.getMode() == 2) jsonUpdate.addProperty("hand", true);
                else if (segment.inventoryUpdate.getMode() == 1) jsonUpdate.addProperty("full", true);
        ...
        public Segment deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
            ...
            String classString = jsonObject.get("class")
                .getAsString();
            ...
            if (!(segment instanceof SegmentSpecialBlock)) {
                ...
                if (jsonObject.has("condition")) {
                    segment.condition = new Condition(
                ...
                if (jsonObject.has("priority")) {
                    segment.priority = Priority.valueOf(
                        jsonObject.get("priority")
                            .getAsString());
        ...
        private SegmentBlock deserializeBlock(JsonObject json, JsonDeserializationContext context) {
            ...
            if (json.has("clientUpdate")) {
                segment.clientUpdate = new ClientBlockUpdate(
                    (Volume) context.deserialize(
                        json.get("clientUpdate")
                            .getAsJsonObject()
                            .get("coords"),
                        Volume.class));
        ...
        private SegmentSpecialBlock deserializeSpecialBlock(JsonObject json, JsonDeserializationContext context) {
            ...
            if (json.has("isAlwaysBreakable")) {
                segment.isAlwaysBreakable = json.get("isAlwaysBreakable")
                    .getAsBoolean();
        ...
        private SegmentItem deserializeItem(JsonObject json, JsonDeserializationContext context) {
            ...
            if (json.has("damage")) {
                segment.damage = json.get("damage")
                    .getAsInt();
                json.remove("damage");
            }

            if (json.has("isAdjacent")) {
                segment.isAdjacent = json.get("isAdjacent")
                    .getAsBoolean();
                json.remove("isAdjacent");
            }

            if (json.has("clientUpdate")) {
                JsonObject jsonClientUpdate = json.get("clientUpdate")
                    .getAsJsonObject();
                segment.clientUpdate = new ClientBlockUpdate(
                    (Volume) context.deserialize(jsonClientUpdate.get("coords"), Volume.class));
                if (jsonClientUpdate.has("directional")) {
                    segment.directionalClientUpdate = jsonClientUpdate.get("directional")
                        .getAsBoolean();
                }
                json.remove("clientUpdate");
            }
            ...
            if (json.has("inventoryUpdate")) {
                JsonObject jsonItemUpdate = json.get("inventoryUpdate")
                    .getAsJsonObject();
                int mode = jsonItemUpdate.get("hand")
                    .getAsBoolean() ? 2
                        : jsonItemUpdate.get("full")
                            .getAsBoolean() ? 1 : 0;
                if (mode > 0) segment.inventoryUpdate = new ClientInventoryUpdate(mode);
        ...
        private SegmentTileEntity deserializeTileEntity(JsonObject json, JsonDeserializationContext context) {
            ...
            segment.retainsOwner = json.getAsJsonObject()
                .get("retainsOwner")
                .getAsBoolean();
        ...
        public List<T> get(Class<?> clazz) {
            ...
            if (usableSegments.size() > 1) {
                ...
                for (Segment segment : usableSegments) {
                    if (highestPriority.ordinal() < segment.getPriority()
                        .ordinal()) {
