...
public class TCWarpHandler
{
  ...
  public static boolean tcReflect()
  {
    ...
    {
      MyTown.instance.LOG.warn("Could not reflect into thaumcraft.common.Thaumcraft to get warpNormal mappings, attempting older reflection");
      e.printStackTrace();
      try
      {
        Class tc = Class.forName("thaumcraft.common.Thaumcraft");
      ...
      {
        MyTown.instance.LOG.warn("Failed to reflect into thaumcraft.common.Thaumcraft to get warpNormal mapping");
        e.printStackTrace();
        return false;
      }
    }
  ...
  public static int getTotalWarp(EntityPlayer pPlayer)
  {
    ...
    {
      return ((warpPermanent != null && warpPermanent.get(pPlayer.getDisplayName()) != null) ? warpPermanent.get(pPlayer.getDisplayName()) : 0) +
          (warpNormal.get(pPlayer.getDisplayName()) != null ? warpNormal.get(pPlayer.getDisplayName()) : 0) +
  ...
  {
    int w = 0;
    if (pPlayer.getCurrentEquippedItem() != null && pPlayer.getCurrentEquippedItem().getItem() instanceof IWarpingGear)
      w += ((IWarpingGear)pPlayer.getCurrentEquippedItem().getItem()).getWarp(pPlayer.getCurrentEquippedItem(), pPlayer);
    IInventory baubles = BaublesApi.getBaubles(pPlayer);
    for (int i = 0; i < 4; i++)
    {
      if (pPlayer.inventory.getStackInSlot(i) != null && pPlayer.inventory.getStackInSlot(i).getItem() instanceof IWarpingGear)
        w += ((IWarpingGear)pPlayer.inventory.getStackInSlot(i).getItem()).getWarp(pPlayer.inventory.getStackInSlot(i), pPlayer);
      if (baubles != null && baubles.getStackInSlot(i) != null && baubles.getStackInSlot(i).getItem() instanceof IWarpingGear)
        w += ((IWarpingGear)baubles.getStackInSlot(i).getItem()).getWarp(baubles.getStackInSlot(i), pPlayer);
    }
    return w;
  }
}
