...
public class MyTownDatasource extends DatasourceSQL {
    ...
    public boolean loadAll() {
        return loadWorlds() && loadTowns()
            && loadRanks()
            && loadBlocks()
            && loadResidents()
            && loadPlots()
            && /* loadNations() && */ loadTownFlags()
            && loadPlotFlags()
            && loadBlockWhitelists()
            && loadTownInvites()
            && loadBlockOwners()
            ...
            while (rs.next()) {
                ...
                town.setSpawn(
                    new Teleport(
                        ...
                        rs.getFloat("spawnY"),
                        rs.getFloat("spawnZ"),
                        rs.getFloat("cameraYaw"),
                        rs.getFloat("cameraPitch")));
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    PreparedStatement removeFlag = prepare(
                        "DELETE FROM " + prefix + "TownFlags WHERE townName=? AND name=?",
                        true);
                ...
                try {
                    ...
                    PreparedStatement updateFlag = prepare(
                        "UPDATE " + prefix + "TownFlags SET serializedValue=? WHERE townName=? AND name=?",
                        false);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    LOG.error(ExceptionUtils.getStackTrace(ex));
                    PreparedStatement removeFlag = prepare(
                        "DELETE FROM " + prefix + "PlotFlags WHERE plotID=? AND name=?",
                        true);
                ...
                try {
                    ...
                    LOG.error(ExceptionUtils.getStackTrace(ex));
                    PreparedStatement updateFlag = prepare(
                        "UPDATE " + prefix + "PlotFlags SET serializedValue=? WHERE plotID=? AND name=?",
                        false);
                    updateFlag.setString(1, flagType.serializeValue(flagType.defaultValue));
    ...
    protected boolean loadBlockWhitelists() {
        try {
            ...
            while (rs.next()) {
                ...
                BlockWhitelist bw = new BlockWhitelist(
                    rs.getInt("dim"),
                    rs.getInt("x"),
                    rs.getInt("y"),
                    rs.getInt("z"),
        ...
        try {
            if (getUniverse().towns.contains(town)) { // Update
                PreparedStatement updateStatement = prepare(
                    "UPDATE " + prefix
                        + "Towns SET name=?, spawnDim=?, spawnX=?, spawnY=?, spawnZ=?, cameraYaw=?, cameraPitch=?, extraBlocks=?, maxPlots=?, extraFarClaims=? WHERE name=?",
                    true);
                ...
                if (town.getOldName() == null) updateStatement.setString(11, town.getName());
                else updateStatement.setString(11, town.getOldName());
                ...
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix
                        + "Towns (name, spawnDim, spawnX, spawnY, spawnZ, cameraYaw, cameraPitch, isAdminTown, extraBlocks, maxPlots, extraFarClaims) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    true);
    ...
    public boolean saveBlock(TownBlock block) {
        ...
        try {
            if (getUniverse().blocks.contains(block)) { // Update
                ...
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix
                        + "Blocks (dim, x, z, isFarClaim, pricePaid, townName) VALUES (?, ?, ?, ?, ?, ?)",
                    true);
            ...
            if (getUniverse().ranks.contains(rank)) { // Update
                try {
                    ...
                    PreparedStatement s = prepare(
                        "UPDATE " + prefix + "Ranks SET type=?, name=? WHERE name=? AND townName=?",
                        true);
                    s.setString(
                        1,
                        rank.getType()
                    ...
                    if (!rank.permissionsContainer.isEmpty()) {
                        s = prepare(
                            "INSERT INTO " + prefix + "RankPermissions(node, rank, townName) VALUES(?, ?, ?)",
                            true);
                ...
                try {
                    ...
                    PreparedStatement insertRankStatement = prepare(
                        "INSERT INTO " + prefix + "Ranks (name, townName, type) VALUES(?, ?, ?)",
                        true);
                    insertRankStatement.setString(1, rank.getName());
                    insertRankStatement.setString(
                        2,
                        rank.getTown()
                            .getName());
                    insertRankStatement.setString(
                    ...
                    if (!rank.permissionsContainer.isEmpty()) {
                        ...
                        for (String perm : rank.permissionsContainer) {
                            ...
                            insertRankPermStatement.setString(
                                3,
                                rank.getTown()
                                    .getName());
    ...
    public boolean saveRankPermission(Rank rank, String perm) {
        LOG.debug(
            "Saving RankPermission {} for Rank {} in Town {}",
            perm,
            rank.getName(),
            rank.getTown()
                .getName());
    ...
    public boolean saveResident(Resident resident) {
        ...
        try {
            if (getUniverse().residents.contains(resident.getUUID())) { // Update
                PreparedStatement updateStatement = prepare(
                    "UPDATE " + prefix + "Residents SET name=?, lastOnline=?, extraBlocks=?, fakePlayer=? WHERE uuid=?",
                    true);
                updateStatement.setString(1, resident.getPlayerName());
                updateStatement.setLong(
                    2,
                    resident.getLastOnline()
                        .getTime() / 1000L); // Stupid hack...
                ...
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix
                        + "Residents (uuid, name, joined, lastOnline, extraBlocks, fakePlayer) VALUES(?, ?, ?, ?, ?, ?)",
                    true);
                insertStatement.setString(
                    1,
                    resident.getUUID()
    ...
    public boolean savePlot(Plot plot) {
        ...
        try {
            if (getUniverse().plots.contains(plot)) { // Update
                ...
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix
                        + "Plots (name, dim, x1, y1, z1, x2, y2, z2, townName) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    true);
                ...
                if (generatedKeys.next()) plot.setDbID(generatedKeys.getInt(1));
    ...
    public boolean saveFlag(Flag flag, Plot plot) {
        ...
        try {
            if (plot.flagsContainer.contains(flag.flagType)) {
                ...
                // Insert
                PreparedStatement s = prepare(
                    "INSERT INTO " + prefix + "PlotFlags(name, serializedValue, plotID) VALUES(?, ?, ?)",
                    true);
    ...
    public boolean saveFlag(Flag flag, Town town) {
        ...
        try {
            if (town.flagsContainer.contains(flag.flagType)) {
                ...
                // Insert
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix + "TownFlags(name,  serializedValue, townName) VALUES(?, ?, ?)",
                    true);
    ...
    public boolean saveBlockWhitelist(BlockWhitelist bw, Town town) {
        try {
            if (!town.blockWhitelistsContainer.contains(bw)) {
                PreparedStatement insertStatement = prepare(
                    "INSERT INTO " + prefix
                        + "BlockWhitelists(dim, x, y, z, flagName, townName) VALUES(?, ?, ?, ?, ?, ?)",
                    true);
                ...
                if (keys.next()) bw.setDbID(keys.getInt(1));
    ...
    public boolean saveSelectedTown(Resident res, Town town) {
        try {
            if (res.townsContainer.isSelectedTownSaved) {
                PreparedStatement statement = prepare(
                    "UPDATE " + prefix + "SelectedTown SET townName=? WHERE resident=?",
                    true);
                ...
                PreparedStatement statement = prepare(
                    "INSERT INTO " + prefix + "SelectedTown(resident, townName) VALUES(?, ?)",
                    true);
                statement.setString(
                    1,
                    res.getUUID()
    ...
    public boolean saveTownInvite(Resident res, Town town) {
        try {
            if (!res.townInvitesContainer.contains(town)) {
                PreparedStatement s = prepare(
                    "INSERT INTO " + prefix + "TownInvites(resident, townName) VALUES(?, ?)",
                    true);
                s.setString(
                    1,
                    res.getUUID()
    ...
    public boolean saveBlockOwner(Resident res, int dim, int x, int y, int z) {
        try {
            PreparedStatement s = prepare(
                "INSERT INTO " + prefix + "BlockOwners(resident, dim, x, y, z) VALUES(?, ?, ?, ?, ?)",
                false);
            s.setString(
                1,
                res.getUUID()
    ...
    public boolean saveTownBank(Bank bank) {
        try {
            if (getUniverse().banks.contains(bank)) {
                PreparedStatement s = prepare(
                    "UPDATE " + prefix + "TownBanks SET amount=?, daysNotPaid=? WHERE townName=?",
                    false);
    ...
    public boolean linkResidentToTown(Resident res, Town town, Rank rank) {
        try {
            PreparedStatement s = prepare(
                "INSERT INTO " + prefix + "ResidentsToTowns (resident, town, rank) VALUES(?, ?, ?)",
                true);
            s.setString(
                1,
                res.getUUID()
            ...
            LOG.error(
                "Failed to link Resident {} ({}) with Town {}",
                res.getPlayerName(),
                res.getUUID(),
                town.getName());
            LOG.error(ExceptionUtils.getStackTrace(e));
    ...
    public boolean unlinkResidentFromTown(Resident res, Town town) {
        try {
            PreparedStatement s = prepare(
                "DELETE FROM " + prefix + "ResidentsToTowns WHERE resident = ? AND town = ?",
                true);
            s.setString(
                1,
                res.getUUID()
            ...
            LOG.error(
                "Failed to unlink Resident {} ({}) with Town {}",
                e,
                res.getPlayerName(),
                res.getUUID(),
                town.getName());
    ...
    public boolean updateResidentToTownLink(Resident res, Town town, Rank rank) {
        try {
            PreparedStatement s = prepare(
                "UPDATE " + prefix + "ResidentsToTowns SET rank = ? WHERE resident = ? AND town = ?",
                true);
            s.setString(1, rank.getName());
            s.setString(
                2,
            ...
            LOG.error(
                "Failed to update link between Resident {} ({}) with Town {}",
                e,
                res.getPlayerName(),
                res.getUUID(),
                town.getName());
     ...
     * try {
     * PreparedStatement s = prepare("INSERT INTO " + prefix + "TownsToNations (town, nation, rank) VALUES(?, ?, ?);",
     * true);
     ...
     * try {
     * PreparedStatement s = prepare("UPDATE " + prefix + "TownsToNations SET rank = ? WHERE town = ?, nation = ?",
     * true);
    ...
    public boolean linkResidentToPlot(Resident res, Plot plot, boolean isOwner) {
        try {
            PreparedStatement s = prepare(
                "INSERT INTO " + prefix + "ResidentsToPlots(resident, plotID, isOwner) VALUES(?, ?, ?)",
                true);
            s.setString(
                1,
                res.getUUID()
            ...
            LOG.error(
                "Failed to link {} to plot {} in town {}",
                res.getPlayerName(),
                plot.getName(),
                plot.getTown()
                    .getName());
    ...
    public boolean unlinkResidentFromPlot(Resident res, Plot plot) {
        try {
            PreparedStatement s = prepare(
                "DELETE FROM " + prefix + "ResidentsToPlots WHERE resident=? AND plotID=?",
                true);
            s.setString(
                1,
                res.getUUID()
            ...
            LOG.error(
                "Failed to unlink {} to plot {} in town {}",
                res.getPlayerName(),
                plot.getName(),
                plot.getTown()
                    .getName());
    ...
    public boolean updateResidentToPlotLink(Resident res, Plot plot, boolean isOwner) {
        try {
            PreparedStatement s = prepare(
                "UPDATE " + prefix + "ResidentsToPlots SET isOwner=? WHERE resident=? AND plotID=?",
                true);
            ...
            LOG.error(
                "Failed to update link {} to plot {} in town {}",
                res.getPlayerName(),
                plot.getName(),
                plot.getTown()
                    .getName());
    ...
    public boolean deleteTown(Town town) {
        try {
            ...
            for (Resident res : town.residentsMap.keySet()) {
                if (res.townsContainer.getMainTown() == town) deleteSelectedTown(res);
                res.townsContainer.remove(town);
            }
    ...
    public boolean deleteBlock(TownBlock block) {
        try {
            ...
            PreparedStatement deleteBlockStatement = prepare(
                "DELETE FROM " + prefix + "Blocks WHERE dim=? AND x=? AND z=?",
                true);
    ...
    public boolean deleteRank(Rank rank) {
        try {
            ...
            PreparedStatement deleteRankStatement = prepare(
                "DELETE FROM " + prefix + "Ranks WHERE name=? AND townName=?",
                true);
            deleteRankStatement.setString(1, rank.getName());
            deleteRankStatement.setString(
                2,
                rank.getTown()
                    .getName());
    ...
    public boolean deleteResident(Resident resident) {
        try {
            ...
            PreparedStatement deleteResidentStatement = prepare(
                "DELETE FROM " + prefix + "Residents WHERE uuid=?",
                true);
            deleteResidentStatement.setString(
                1,
                resident.getUUID()
                    .toString());
    ...
    public boolean deleteTownInvite(Resident res, Town town, boolean response) {
        try {
            PreparedStatement s = prepare(
                "DELETE FROM " + prefix + "TownInvites WHERE resident=? AND townName=?",
                true);
            s.setString(
                1,
                res.getUUID()
    ...
    public boolean deleteFlag(Flag flag, Town town) {
        try {
            PreparedStatement deleteFlagStatement = prepare(
                "DELETE FROM " + prefix + "TownFlags WHERE name=? AND townName=?",
                true);
    ...
    public boolean deleteFlag(Flag flag, Plot plot) {
        try {
            PreparedStatement deleteFlagStatement = prepare(
                "DELETE FROM " + prefix + "PlotFlags WHERE name=? AND plotID=?",
                true);
            deleteFlagStatement.setString(1, flag.flagType.toString());
    ...
    public boolean deleteRankPermission(Rank rank, String perm) {
        try {
            PreparedStatement s = prepare(
                "DELETE FROM " + prefix + "RankPermissions WHERE node = ? AND rank = ? AND townName = ?",
                true);
            s.setString(1, perm);
            s.setString(2, rank.getName());
            s.setString(
            ...
            LOG.error(
                "Failed to add permission ({}) to Rank ({}) in Town ({})",
                perm,
                rank.getName(),
                rank.getTown()
                    .getName());
        ...
        for (Town town : getUniverse().towns) {
            for (FlagType type : FlagType.values()) {
                if (type.isTownPerm && !town.flagsContainer.contains(type)) {
                    ...
                    LOG.info(
                        "Flag {} in town {} got created because of the settings.",
        ...
        for (Plot plot : getUniverse().plots) {
            for (FlagType type : FlagType.values()) {
                if (type.isPlotPerm && !plot.flagsContainer.contains(type)) {
                    ...
                    LOG.info(
                        "Flag {} in plot {} of town {} got created because of the settings.",
                        type.name.toLowerCase(),
                        plot.getName(),
                        plot.getTown()
                            .getName());
