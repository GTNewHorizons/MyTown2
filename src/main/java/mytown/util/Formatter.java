    ...
    public static void sendMap(Resident res, int dim, int cx, int cz) {
        ...
        for (int z = cz - heightRad; z <= cz + heightRad; z++) {
            ...
            for (int x = cx - widthRad; x <= cx + widthRad; x++) {
                ...
                if (mid) {
                    extraBuilder.setColor(
                        ownTown ? EnumChatFormatting.GREEN
                            : isTown ? EnumChatFormatting.RED : EnumChatFormatting.WHITE);
                } else {
                    extraBuilder.setColor(
                        ownTown ? EnumChatFormatting.DARK_GREEN
                            : isTown ? EnumChatFormatting.DARK_RED : EnumChatFormatting.GRAY);
                }
                ...
                if (b != null) {
                    ...
                    extraBuilder.setClickEventRunCommand(
                        "/t info " + b.getTown()
                            .getName());
                }
            }
            ...
            res.getPlayer()
                .addChatMessage(msgBuilder.build());
        }
    }
