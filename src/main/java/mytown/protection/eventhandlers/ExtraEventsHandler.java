...
public class ExtraEventsHandler {
    ...
    public static ExtraEventsHandler getInstance() {
        if (instance == null) instance = new ExtraEventsHandler();
        return instance;
    }
    ...
    public void onExplosion(ExplosionEvent.Start ev) {
        ...
        for (ChunkPos chunk : chunks) {
            TownBlock block = MyTownUniverse.instance.blocks
                .get(ev.world.provider.dimensionId, chunk.getX(), chunk.getZ());
            if (block == null) {
                ...
                if (!(Boolean) block.getTown().flagsContainer.getValue(FlagType.EXPLOSIONS)) {
                    ...
                    String[] arr = new String[] { StringUtils.split(
                        MyTown.instance.LOCAL.getLocalizationMap()
                            .get(FlagType.EXPLOSIONS.getTownNotificationKey()),
                        "|}")[1], ". Pos: ", Integer.toString((int) ev.explosion.explosionX), ",",
                        Integer.toString((int) ev.explosion.explosionY), ",",
                        Integer.toString((int) ev.explosion.explosionZ), " Dim: ", Integer.toString(block.getDim()),
                        " Town: ", block.getTown()
                            .getName() };
                    ...
                    block.getTown()
                        .notifyEveryone(
                            new ChatComponentText(sb.toString())
                                .setChatStyle(new ChatStyle().setColor(ColorUtils.colorMap.get('3'))));
                    return;
                }
            }
        }
