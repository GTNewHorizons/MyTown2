...
import mytown.protection.segment.SegmentSpecialBlock;
...
public class ProtectionManager {
    ...
    public static void check(EntityPlayerMP player) {
        Town town = MyTownUtils.getTownAtPosition(
            player.dimension,
            (int) Math.floor(player.posX) >> 4,
        ...
        boolean tHasPermissionToEnter = ProtectionManager.hasPermission(
            res,
            FlagType.ENTER,
            player.dimension,
        ...
        if (tHasPermissionToEnter) tWarpIsOK = ProtectionManager.checkWarpLevel(
            res,
            player.dimension,
            (int) Math.floor(player.posX),
            (int) Math.floor(player.posY),
        ...
        if (!tHasPermissionToEnter || !tWarpIsOK) {
            if (lastTickPos == null) {
                ...
                || lastTickPos.getZ() != player.posZ
                || lastTickPos.getDim() != player.dimension) {
            ...
            if (lastTickPos != null
                ...
                if (lastTickPos.getDim() == player.dimension) {
                    res.checkLocation(
                        ((int) Math.floor(lastTickPos.getX())) >> 4,
                        ((int) Math.floor(lastTickPos.getZ())) >> 4,
                        ((int) Math.floor(player.posX)) >> 4,
                        ((int) (Math.floor(player.posZ))) >> 4,
                        player.dimension);
                } else {
                    res.checkLocationOnDimensionChanged(
                        (int) (Math.floor(player.posX)),
                        (int) (Math.floor(player.posZ)),
                        player.dimension);
                }
            ...
            if (lastTickPos != null && town != null) {
                Plot currentPlot = town.plotsContainer.get(
                    player.dimension,
                    (int) Math.floor(player.posX),
                    (int) Math.floor(player.posY),
                    (int) Math.floor(player.posZ));
                Plot lastTickPlot = town.plotsContainer.get(
                    lastTickPos.getDim(),
                    (int) Math.floor(lastTickPos.getX()),
                    (int) Math.floor(lastTickPos.getY()),
                ...
                if (currentPlot != null && (lastTickPlot == null || currentPlot != lastTickPlot)) {
                    ...
                    ChatManager.send(
                        player,
                        "mytown.notification.plot.enter",
                        LocalManager.get("mytown.notification.plot.enter.unassigned"));
                }
            }
    ...
    public static void check(TileEntity te) {
        for (SegmentTileEntity segment : segmentsTile.get(te.getClass())) {
            if (!segment.shouldExist(te)) {
                ...
                te.getWorldObj()
                    .setBlock(te.xCoord, te.yCoord, te.zCoord, Blocks.air);
                te.invalidate();
    ...
    public static void checkUsage(ItemStack stack, Resident res, PlayerInteractEvent.Action action, BlockPos bp,
        int face, Event ev) {
        if (!ev.isCancelable()) {
            return;
        }

        for (SegmentItem segment : segmentsItem.get(
            stack.getItem()
                .getClass())) {
    ...
    public static void checkBlockInteraction(Resident res, BlockPos bp, PlayerInteractEvent.Action action, Event ev) {
        ...
        World world = MinecraftServer.getServer()
            .worldServerForDimension(bp.getDim());
    ...
    /**
     * Check warp level against town Warp Setting. Will only check for towns
     * 
     * @param res Resident instance
     ...
     * @return
     */
    public static boolean checkWarpLevel(Resident res, int dim, int x, int y, int z) {
        if (MyTownUniverse.instance.blocks.contains(dim, x >> 4, z >> 4)) {
            Town town = MyTownUniverse.instance.blocks.get(dim, x >> 4, z >> 4)
                .getTown();
            return town.checkWarpLevel(res);
        } else return true;
    }
    ...
    public static boolean isBlockWhitelistValid(BlockWhitelist bw) {
         ...
         * && !checkActivatedBlocks(MinecraftServer.getServer().worldServerForDimension(bw.getDim()).getBlock(bw.getX(),
         * bw.getY(), bw.getZ()),
         * MinecraftServer.getServer().worldServerForDimension(bw.getDim()).getBlockMetadata(bw.getX(), bw.getY(),
         * bw.getZ())))
         * return false;
         * if (bw.getFlagType() == FlagType.MODIFY || bw.getFlagType() == FlagType.ACTIVATE || bw.getFlagType() ==
         * FlagType.USAGE) {
         * TileEntity te = MinecraftServer.getServer().worldServerForDimension(bw.getDim()).getTileEntity(bw.getX(),
         * bw.getY(), bw.getZ());
         * if (te == null)
         * return false;
    ...
    public static void saveBlockOwnersToDB() {
        for (Map.Entry<TileEntity, Resident> set : ProtectionHandlers.instance.ownedTileEntities.entrySet()) {
            MyTown.instance.datasource.saveBlockOwner(
                set.getValue(),
                set.getKey()
