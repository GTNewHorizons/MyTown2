...
import mytown.commands.CommandsOutsider;
import mytown.commands.RankPermissionManager;
...
public class MyTown {
    ...
    public void preInit(FMLPreInitializationEvent ev) {
        ...
        FMLCommonHandler.instance()
            .registerCrashCallable(new DatasourceCrashCallable());
    }

    @EventHandler
    public void postInit(FMLPostInitializationEvent ev) {
        SellSign.SellSignType.instance.register();
        if (Loader.isModLoaded("Thaumcraft")) TCWarpHandler.tcReflect();
    }
    ...
    public WildPermsConfig getWildConfig() {
        for (JsonConfig jsonConfig : jsonConfigs) {
            if (jsonConfig instanceof WildPermsConfig) return (WildPermsConfig) jsonConfig;
        }
        return null;
    }
    ...
    public RanksConfig getRanksConfig() {
        for (JsonConfig jsonConfig : jsonConfigs) {
            if (jsonConfig instanceof RanksConfig) return (RanksConfig) jsonConfig;
        }
        return null;
    }
    ...
    public FlagsConfig getFlagsConfig() {
        for (JsonConfig jsonConfig : jsonConfigs) {
            if (jsonConfig instanceof FlagsConfig) return (FlagsConfig) jsonConfig;
        }
        return null;
    }
    ...
    private void registerHandlers() {
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(VisualsHandler.instance);

        FMLCommonHandler.instance()
            .bus()
            .register(ProtectionHandlers.instance);
    ...
    private void checkConfig() {
        ...
        if (EconomyProxy.isItemEconomy()) {
            String[] split = Config.instance.costItemName.get()
                .split(":");
