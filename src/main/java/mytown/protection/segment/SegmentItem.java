...
public class SegmentItem extends Segment {
    ...
    public boolean shouldInteract(ItemStack item, Resident res, PlayerInteractEvent.Action action, BlockPos bp,
        int face) {
        ...
        if (range == 0) {
            if (!hasPermissionAtLocation(res, dim, x, y, z)) {
                ...
                if (inventoryUpdate != null) inventoryUpdate.send(player);
                return false;
            }
        } else {
