...
public class CommandsAssistant extends Commands {
    ...
    public static CommandResponse claimCommand(ICommandSender sender, List<String> args) {
        ...
        if (args.size() < 1) {
            ...
            for (int x = player.chunkCoordX - Config.instance.distanceBetweenTowns.get(); x
                <= player.chunkCoordX + Config.instance.distanceBetweenTowns.get(); x++) {
                for (int z = player.chunkCoordZ - Config.instance.distanceBetweenTowns.get(); z
                    <= player.chunkCoordZ + Config.instance.distanceBetweenTowns.get(); z++) {
                    Town nearbyTown = MyTownUtils.getTownAtPosition(player.dimension, x, z);
                    if (nearbyTown != null && nearbyTown != town
                        && !nearbyTown.flagsContainer.getValue(FlagType.NEARBY)) {
            ...
            TownBlock block = getUniverse()
                .newBlock(player.dimension, player.chunkCoordX, player.chunkCoordZ, isFarClaim, price, town);
            if (block == null) {
            ...
            for (Iterator<ChunkPos> it = chunks.iterator(); it.hasNext();) {
                ...
                for (int x = chunk.getX() - Config.instance.distanceBetweenTowns.get(); x
                    <= chunk.getX() + Config.instance.distanceBetweenTowns.get(); x++) {
                    for (int z = chunk.getZ() - Config.instance.distanceBetweenTowns.get(); z
                        <= chunk.getZ() + Config.instance.distanceBetweenTowns.get(); z++) {
                        Town nearbyTown = MyTownUtils.getTownAtPosition(player.dimension, x, z);
                        if (nearbyTown != null && nearbyTown != town
                            && !nearbyTown.flagsContainer.getValue(FlagType.NEARBY)) {
                            throw new MyTownCommandException(
            ...
            makeBankPayment(
                ...
                (isFarClaim
                    ...
                    + MathUtils.sumFromNtoM(
                        ...
                        town.townBlocksContainer.size() + chunks.size() - 1) * Config.instance.costAdditionClaim.get());
    ...
    public static CommandResponse promoteCommand(ICommandSender sender, List<String> args) {
        ...
        if (args.get(1)
            .equalsIgnoreCase(mayorRank.getName())) {
    ...
    public static class ModifyRanks {
        ...
        public static CommandResponse ranksAddCommand(ICommandSender sender, List<String> args) {
            ...
            if (town.ranksContainer.contains(args.get(0))) {
                throw new MyTownCommandException(
                    "mytown.cmd.err.ranks.add.already",
                    town.ranksContainer.get(args.get(0)));
            }
