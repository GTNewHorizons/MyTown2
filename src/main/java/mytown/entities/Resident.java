...
public class Resident implements IChatFormat {
    ...
    public void checkLocation(int oldChunkX, int oldChunkZ, int newChunkX, int newChunkZ, int dimension) {
        if (oldChunkX != newChunkX || oldChunkZ != newChunkZ && player != null) {
            ...
            if (oldTownBlock == null && newTownBlock != null || oldTownBlock != null && newTownBlock != null
                && !oldTownBlock.getTown()
                    .getName()
                    .equals(
                        newTownBlock.getTown()
                            .getName())) {
    ...
    public void respawnPlayer() {
        if (townsContainer.getMainTown() != null) {
            townsContainer.getMainTown()
                .sendToSpawn(this);
            return;
        }

        ChunkCoordinates spawn = player.getBedLocation(player.dimension);
        if (spawn == null) spawn = player.worldObj.getSpawnPoint();
    ...
    public void knockbackPlayerToBorder(Town town) {
        if (this.player != null) {
            ...
            boolean ok = false;
            while (!ok) {
                while (!town.hasPermission(this, FlagType.ENTER, player.dimension, x, y, z)
                    && town.isPointInTown(player.dimension, x, z)) x++;
                x += 3;

                while (player.worldObj.getBlock(x, y, z) != Blocks.air
                    && player.worldObj.getBlock(x, y + 1, z) != Blocks.air
                    && y < 256) y++;

                if (town.hasPermission(this, FlagType.ENTER, player.dimension, x, y, z)
                    || !town.isPointInTown(player.dimension, x, z)) ok = true;
            }
    ...
    public Date getLastOnline() {
        if (this.player != null) {
            lastOnline = new Date(); // TODO Do we REALLY need to update this each time its received, or can we do this
                                     // better?
        }
        return lastOnline;
    }
