...
public class VisualsHandler {
    ...
    public void tick(TickEvent.ServerTickEvent ev) {
        ...
        if (!markedBlocks.isEmpty()) {
            for (Iterator<VisualObject> visualObjectIterator = markedBlocks.iterator(); visualObjectIterator
                ...
                for (Iterator<BlockCoords> blockCoordsIterator = visualObject.blockCoords
                    ...
                    if (!coord.packetSent) {
                        S23PacketBlockChange packet = new S23PacketBlockChange(
                            ...
                            MinecraftServer.getServer()
                                .worldServerForDimension(coord.dim));
                    ...
                    if (coord.deleted) {
                        S23PacketBlockChange packet = new S23PacketBlockChange(
                            ...
                            MinecraftServer.getServer()
                                .worldServerForDimension(coord.dim));
                        packet.field_148883_d = MinecraftServer.getServer()
                            .worldServerForDimension(coord.dim)
                            .getBlock(coord.x, coord.y, coord.z);
                        packet.field_148884_e = MinecraftServer.getServer()
                            .worldServerForDimension(coord.dim)
                ...
                if (visualObject.blockCoords.isEmpty()) visualObjectIterator.remove();
            }
        }
    }
    ...
    public void markCorners(int selectionX1, int selectionY1, int selectionZ1, int selectionX2, int selectionY2,
        ...
        blockList.add(
            new BlockCoords(
                selectionX1 + (selectionX1 > selectionX2 ? -1 : 1),
                selectionY1,
                selectionZ1,
                dim,
        ...
        blockList.add(
            new BlockCoords(
                selectionX2 + (selectionX1 > selectionX2 ? 1 : -1),
                selectionY2,
                selectionZ2,
                dim,
                Blocks.redstone_block));
        blockList.add(
            new BlockCoords(
                selectionX1 + (selectionX1 > selectionX2 ? -2 : 2),
                selectionY1,
                selectionZ1,
                dim,
                Blocks.redstone_block));
        blockList.add(
            new BlockCoords(
                selectionX2 + (selectionX1 > selectionX2 ? 2 : -2),
                selectionY2,
                selectionZ2,
                dim,
                Blocks.redstone_block));
        ...
        blockList.add(
            new BlockCoords(
                selectionX2,
                selectionY2,
                selectionZ2 + (selectionZ1 > selectionZ2 ? 1 : -1),
                dim,
                Blocks.redstone_block));
        blockList.add(
            new BlockCoords(
                selectionX1,
                selectionY1,
                selectionZ1 + (selectionZ1 > selectionZ2 ? -1 : 1),
                dim,
                Blocks.redstone_block));
        blockList.add(
            new BlockCoords(
                selectionX2,
                selectionY2,
                selectionZ2 + (selectionZ1 > selectionZ2 ? 2 : -2),
                dim,
                Blocks.redstone_block));
        blockList.add(
            new BlockCoords(
                selectionX1,
                selectionY1,
                selectionZ1 + (selectionZ1 > selectionZ2 ? -2 : 2),
                dim,
                Blocks.redstone_block));
        ...
        if (selectionY1 != selectionY2) {
            ...
            blockList.add(
                new BlockCoords(
                    selectionX1,
                    selectionY1 + (selectionY1 > selectionY2 ? -1 : 1),
                    selectionZ1,
                    dim,
                    Blocks.redstone_block));
            blockList.add(
                new BlockCoords(
                    selectionX2,
                    selectionY2 + (selectionY1 > selectionY2 ? 1 : -1),
                    selectionZ2,
                    dim,
                    Blocks.redstone_block));
            blockList.add(
                new BlockCoords(
                    selectionX1,
                    selectionY1 + (selectionY1 > selectionY2 ? -2 : 2),
                    selectionZ1,
                    dim,
                    Blocks.redstone_block));
            blockList.add(
                new BlockCoords(
                    selectionX2,
                    selectionY2 + (selectionY1 > selectionY2 ? 2 : -2),
                    selectionZ2,
                    dim,
                    Blocks.redstone_block));
        }
    ...
    public synchronized void unmarkBlocks(EntityPlayerMP caller, Object key) {
        for (VisualObject visualObject : markedBlocks) {
            if (visualObject.player == caller && visualObject.object == key) {
                for (BlockCoords blockCoords : visualObject.blockCoords) blockCoords.deleted = true;
            }
        }
