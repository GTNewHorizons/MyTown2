...
public abstract class Commands {
    ...
    public static Town getTownFromResident(Resident res) {
        ...
        if (town == null) throw new MyTownCommandException("mytown.cmd.err.partOfTown");
        return town;
    }

    public static Town getTownFromName(String name) {
        Town town = getUniverse().towns.get(name);
        if (town == null) throw new MyTownCommandException("mytown.cmd.err.town.missing", name);
        return town;
    }
    ...
    public static Resident getResidentFromName(String playerName) {
        ...
        if (res == null) throw new MyTownCommandException("mytown.cmd.err.resident.missing", playerName);
        return res;
    }
    ...
    public static Plot getPlotAtResident(Resident res) {
        ...
        if (plot == null) throw new MyTownCommandException("mytown.cmd.err.plot.notInPlot");
        return plot;
    }

    public static List<Town> getInvitesFromResident(Resident res) {
        if (res.townInvitesContainer.isEmpty()) throw new MyTownCommandException("mytown.cmd.err.invite.missing");
    ...
    public static Flag getFlagFromType(Flag.Container flagsContainer, FlagType flagType) {
        ...
        if (flag == null) throw new MyTownCommandException("mytown.cmd.err.flagNotExists", flagType.toString());
        return flag;
    }
    ...
    public static TownBlock getBlockAtResident(Resident res) {
        TownBlock block = getUniverse().blocks
            .get(res.getPlayer().dimension, ((int) res.getPlayer().posX) >> 4, ((int) res.getPlayer().posZ >> 4));
        if (block == null) throw new MyTownCommandException(
            "mytown.cmd.err.claim.missing",
            res.townsContainer.getMainTown()
                .getName());
        return block;
    }
    ...
    public static Plot getPlotAtPosition(int dim, int x, int y, int z) {
        ...
        if (town == null) throw new MyTownCommandException("mytown.cmd.err.blockNotInPlot");
        Plot plot = town.plotsContainer.get(dim, x, y, z);
        if (plot == null) throw new MyTownCommandException("mytown.cmd.err.blockNotInPlot");
        return plot;
    }
    ...
    public static void makePayment(EntityPlayer player, int amount) {
        ...
        if (!EconomyProxy.getEconomy()
            .takeMoneyFromPlayer(player, amount)) {
