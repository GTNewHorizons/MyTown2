...
public class ProtectionHandlers {
    ...
    public void onAnyBlockPlacement(EntityPlayer player, BlockEvent.PlaceEvent ev) {
        ...
        if (player instanceof FakePlayer) {
            if (!ProtectionManager
                .getFlagValueAtLocation(FlagType.FAKERS, ev.world.provider.dimensionId, ev.x, ev.y, ev.z)) {
                ev.setCanceled(true);
            }
        } else {
            ...
            if (!MyTownUniverse.instance.blocks.contains(ev.world.provider.dimensionId, ev.x >> 4, ev.z >> 4)) {
                ...
                if (!ProtectionManager
                    .hasPermission(res, FlagType.MODIFY, ev.world.provider.dimensionId, ev.x, ev.y, ev.z)) {
                    ev.setCanceled(true);
                    return;
                }
            }
            ...
            if (ev.block instanceof ITileEntityProvider && ev.itemInHand != null) {
                TileEntity te = ((ITileEntityProvider) ev.block).createNewTileEntity(
                    MinecraftServer.getServer()
                        .worldServerForDimension(ev.world.provider.dimensionId),
                    ev.itemInHand.getItemDamage());
                if (te != null && ProtectionManager.isOwnable(te.getClass())) {
                    ThreadPlacementCheck thread = new ThreadPlacementCheck(
                        res,
                        ev.x,
                        ev.y,
                        ev.z,
    ...
    public void onEntityInteract(EntityInteractEvent ev) {
        ...
        if (!ev.isCanceled() && ev.entityPlayer.getHeldItem() != null) {
            ...
            ProtectionManager
                .checkUsage(ev.entityPlayer.getHeldItem(), res, PlayerInteractEvent.Action.RIGHT_CLICK_AIR, bp, -1, ev);
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent ev) {
        ...
        if (ev.entityPlayer.getHeldItem() != null) {
            ProtectionManager
                .checkUsage(ev.entityPlayer.getHeldItem(), res, ev.action, createBlockPos(ev), ev.face, ev);
        }
        if (!ev.isCanceled()) {
            ProtectionManager.checkBlockInteraction(
                res,
                new BlockPos(ev.x, ev.y, ev.z, ev.world.provider.dimensionId),
                ev.action,
                ev);
        }
    ...
    public void onAE2PartPlace(AE2PartPlaceEvent ev) {
        ...
        if (ev.player.getHeldItem() != null) {
            ProtectionManager.checkUsage(
                ev.player.getHeldItem(),
                res,
    ...
    public void onModifyBlock(ModifyBlockEvent ev) {
        ...
        if (!ProtectionManager
            .getFlagValueAtLocation(FlagType.MODIFY, ev.world.provider.dimensionId, ev.x, ev.y, ev.z)) {
            ev.setCanceled(true);
            return;
        }
    }
    ...
    public void onBlockTrample(BlockTrampleEvent ev) {
        if (ev.world.isRemote || ev.isCanceled()) return;
        ...
        if (!(entity instanceof EntityPlayer)) {
            ...
            else res = ProtectionManager.getOwner(entity);
        }
    ...
    public void onPlayerBreaksBlock(BlockEvent.BreakEvent ev) {
        ...
        if (ev.getPlayer() instanceof FakePlayer) {
            ...
            if (!MyTownUniverse.instance.blocks.contains(ev.world.provider.dimensionId, ev.x >> 4, ev.z >> 4)) {
                ...
                if (!ProtectionManager.checkBlockBreak(ev.block)) {
                    if (!ProtectionManager
                        .hasPermission(res, FlagType.MODIFY, ev.world.provider.dimensionId, breakBox)) {
                        ev.setCanceled(true);
                        return;
                    }
                }
            ...
            if (ev.getPlayer()
                .getHeldItem() != null) {
                ProtectionManager.checkBreakWithItem(
                    ev.getPlayer()
                        .getHeldItem(),
                    res,
                    new BlockPos(ev.x, ev.y, ev.z, ev.world.provider.dimensionId),
    ...
    public void onItemPickup(EntityItemPickupEvent ev) {
        ...
        Resident res = MyTownUniverse.instance.getOrMakeResident(ev.entityPlayer);
        if (!ProtectionManager.hasPermission(
            res,
            FlagType.PICKUP,
            ev.item.dimension,
            (int) Math.floor(ev.item.posX),
            (int) Math.floor(ev.item.posY),
            (int) Math.floor(ev.item.posZ))) {
            ev.setCanceled(true);
        }
    }
    ...
    public void onPlayerChangedDimension(PlayerEvent.PlayerChangedDimensionEvent ev) {
        ...
        if (!ProtectionManager.hasPermission(
            ...
            FlagType.ENTER,
            ev.player.dimension,
            (int) Math.floor(ev.player.posX),
            (int) Math.floor(ev.player.posY),
            (int) Math.floor(ev.player.posZ))) {
